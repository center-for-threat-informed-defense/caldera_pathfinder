...
async def enable(services):
    ...
    nmap_installed = await services.get("app_svc").validate_requirement(
        "nmap",
        dict(type="installed_program", command="nmap --version", version="0.0.0"),
    )
    await services.get("data_svc").apply("vulnerabilityreports")
    pathfinder_gui = PathfinderGUI(
        services=services,
        name=name,
    ...
    app.router.add_static(
        "/pathfinder", "plugins/pathfinder/static/", append_version=True
    )
    ...
    app.router.add_route(
        "GET", "/plugin/pathfinder/download", pathfinder_gui.download_report
    )
    app.router.add_route("*", "/plugin/pathfinder/api", pathfinder_gui.pathfinder_core)
    app.router.add_route(
        "POST", "/plugin/pathfinder/upload", pathfinder_gui.store_report
    )
    settings.init(
        dict(
            name=name,
    ...
    init_dirs(
        [
            os.path.join(settings.data_dir, "abilities"),
